...
with open('books.csv','r') as csvfile:
    ...
    restrictions = {
        1: lambda row: float(row[7]) <= 150,  #До 150 рублей
        ...
        4: lambda row: float(row[7]) <= 200,  #До 200 рублей
        5: lambda row: True,  #Нет
        6: lambda row: float(row[7]) >= 150,  #От 150 рублей
        ...
        9: lambda row: float(row[7]) >= 200,  #От 200 рублей
